...
void zendump_array_dump(zend_array *arr, int level)
{
	...
	php_printf("array(%u) at addr(0x" ZEND_XLONG_FMT ") refcount(%u) hash(%u,%u) bucket(%u,%u) data(0x" ZEND_XLONG_FMT ")\n", arr->nNumOfElements, arr, arr->gc.refcount, hashSize, hashUsed, arr->nTableSize, arr->nNumUsed, arr->arData);
}
...
again:
	...
	switch(Z_TYPE_P(val))
	{
		...
		case IS_STRING:
			...
			php_printf("\" at addr(0x" ZEND_XLONG_FMT ") refcount(%u)\n", Z_STR_P(val), Z_REFCOUNTED_P(val) ? Z_REFCOUNT_P(val) : 1);
			break;
		case IS_ARRAY:
			zendump_array_dump(val->value.arr, level + 1);
			break;
		case IS_OBJECT: {
			zend_string *class_name = Z_OBJ_HANDLER_P(val, get_class_name)(Z_OBJ_P(val));
			php_printf("object(%s) at addr(0x" ZEND_XLONG_FMT ") refcount(%u)\n", ZSTR_VAL(class_name), Z_OBJ_P(val), Z_REFCOUNT_P(val));
			zend_string_release(class_name);
			break;
		}
		case IS_RESOURCE: {
			const char *type_name = zend_rsrc_list_get_rsrc_type(Z_RES_P(val));
			php_printf("resource at addr(0x" ZEND_XLONG_FMT ") with data(0x" ZEND_XLONG_FMT ") of type (%s) refcount(%u)\n", Z_RES_P(val), Z_RES_VAL_P(val), type_name ? type_name : "unknown", Z_REFCOUNT_P(val));
			break;
		}
		case IS_REFERENCE:
			php_printf("reference(%u) at addr(0x" ZEND_XLONG_FMT ") with ", Z_REFCOUNT_P(val), Z_REF_P(val));
			val = Z_REFVAL_P(val);
			goto again;
		case IS_INDIRECT:
			PHPWRITE("-> ", 3);
...
PHP_FUNCTION(zendump)
{
	...
	for (i = 0; i < argc; i++) {
		zendump_zval_dump(&args[i], 1);
	}
}

PHP_FUNCTION(zendump_symbol_table)
{
	...
	zendump_array_dump(prev->symbol_table, 0);
}
...
const zend_function_entry zendump_functions[] = {
	...
	PHP_FE(zendump_symbol_table, NULL)
	PHP_FE(zendump_vars, NULL)
	PHP_FE(zendump_literals, NULL)
	PHP_FE(zendump_opcodes, NULL)
